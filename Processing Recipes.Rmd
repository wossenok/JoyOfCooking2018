...
```{r}
path.to.recipes <- "/Users/j70195/Documents/JoyOfCooking2018/RecipeTables"
```
...
```{r}
nonconforming.files2 <- c()
for (recipe in nonconforming.files) {
    file.name <- paste(path.to.recipes, '/', recipe, sep = '')
    current <- NULL
    #read the first line
    connection <- file(file.name)
    ...
    #but here we have to do this manually.
    first.tokens <- gsub('\"', '', first.tokens)
    first.tokens <- gsub('Unit', 'Measure', first.tokens)
    if (minimum.headers(first.tokens)) {
        recipe.list[[recipe]] <- read.table(file.name,
                                          header = TRUE,
                                          stringsAsFactors = FALSE)
        ...
        nonconforming.files2 <- c(nonconforming.files2, recipe)
        print(recipe)
        print(first.line)
    }
}
...
nonconforming.files3 <- c()
for (recipe in nonconforming.files2) {
    file.name <- paste(path.to.recipes, '/', recipe, sep = '')
    current <- NULL
    #read the first line
    connection <- file(file.name)
    ...
    if (minimum.headers(first.tokens)) {
        ...
        nonconforming.files3 <- c(nonconforming.files3, recipe)
        print(recipe)
        print(first.line)
    }
}
...
```{r}
nonconforming.files4 <- c()
for (recipe in nonconforming.files3) {
    file.name <- paste(path.to.recipes, '/', recipe, sep = '')
    current <- NULL
    #read the first line
    connection <- file(file.name)
    ...
    first.tokens <- gsub('\"', '', first.tokens)
    first.tokens <- gsub('Unit', 'Measure', first.tokens)
    first.tokens <- gsub('Ingredients', 'Ingredient', first.tokens)
    first.tokens <- gsub('pudding_1936', 'Ingredient', first.tokens)
    first.tokens <- gsub('pudding_2006', 'Ingredient', first.tokens)
    #manually fixed chicken gambo files. Set space between cols and col strngs to tab
    ...
    if (minimum.headers(first.tokens)) {
            ...
            print('bad')
            nonconforming.files4 <- c(nonconforming.files4, recipe)
            print(recipe)
            print(first.line)
        }
}
...
for(idx in 1:length(recipe.files)) {
  file.name <- recipe.files[idx]
    tbl <- recipe.list[[file.name]]
  if('Unit' %in% names(tbl)) {
    nbd.idx <- which(names(tbl)=='Unit')
    names(tbl)[nbd.idx] <- expected.columns[2]
    recipe.list[[file.name]] <- tbl
  }
}
```
...
for(idx in 1:length(recipe.files)) {
  file.name <- recipe.files[idx]
    tbl <- recipe.list[[file.name]]
    print(names(tbl))
  if('Recipes' %in% names(tbl)) {
    nbd.idx <- which(names(tbl)=='Recipes')
    names(tbl)[nbd.idx] <- added.columns[3]
    recipe.list[[file.name]] <- tbl
  }
    if ('pudding_1936' %in% names(tbl) | 'pudding_2006' %in% names(tbl)) {
        nbd.idx <- which(names(tbl) == 'Recipes')
        names(tbl)[nbd.idx] <- added.columns[3]
        recipe.list[[file.name]] <- tbl
    }
    if ('NDB' %in% names(tbl)) {
        nbd.idx <- which(names(tbl) == 'NDB')
        names(tbl)[nbd.idx] <- added.columns[1]
        recipe.list[[file.name]] <- tbl
    }
}
...
Some recipes already have `Year` and `Recipe` columns. If we assume these have correct information, we can join directly. 
If not, we parse the file name and add `Year` and `Recipe` columns, using the previously defined utility function.
...
nonconforming.files <- cbind(nonconforming.files, nonconforming.files2)
nonconforming.files <- cbind(nonconforming.files, nonconforming.files3)
nonconforming.files <- cbind(nonconforming.files, nonconforming.files4)
...
#todo check for length of 5 or above
    ...
    tryCatch(test.tbl <- rbind(test.frame, tbl[,join.names]),
             error=function(e) {
                 print(file.name)
                 print(length(names(tbl))) #I added
               #print(head(tbl)) #uncomment
           })
    #if we can bind to the empty table, we can merge this table with
    #the rest
    if(!is.null(test.tbl)) {
